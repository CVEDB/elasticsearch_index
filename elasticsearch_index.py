...
def determine_index(file_path):
    ...
    # Special handling for Cvedb naming conventions
    # If the file path is `in/nuclei-1/output.txt`, the index is `nuclei` not `output`
    is_cvedb_node_output_file = re.match(r"(.*)in\/([-a-z]+)-\d+\/(.*)", file_path)


    # Special handling for cvedb-cli naming conventions
    # If the file path is `run-2023-01-25-110345/nuclei/output.txt`, the index is `nuclei` not `output`
    is_cvedb_cli_output_file = re.match(r"(.*)run\-\d{4}\-\d{2}\-\d{2}(.*)", file_path)

    if is_cvedb_node_output_file:
